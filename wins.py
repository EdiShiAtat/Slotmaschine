...
def flip_horizontal(result):
    # Ergebnisse horizontal spiegeln, um sie in einer besser lesbaren Liste zu halten
    horizontal_values = []
    for value in result.values():
        horizontal_values.append(value)
    # Drehen um 90 Grad, um eine Textdarstellung des Spins in der Reihenfolge zu erhalten
...
def longest_seq(hit):
    ...
    for i in range(len(hit) - 1):
        # Überprüft, ob die Spalten gleich sind
        if hit[i] == hit[i + 1] - 1:
            subSeqLength += 1
